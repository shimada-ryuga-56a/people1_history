class LikesOnSetlistitemInformationsController < ApplicationController
  def create
    ...
    redirect_to event_path(id: setlistitem_information.setlist.event.id)
  end

  def destroy
    setlistitem_information = SetlistitemInformation.find(params[:setlistitem_information_id])
    likes_on_setlistitem_information = current_user.likes_on_setlistitem_information.find_by(setlistitem_information_id: setlistitem_information.id)
    likes_on_setlistitem_information.destroy
    redirect_to event_path(id: setlistitem_information.setlistitem.event.id)
  end
end
